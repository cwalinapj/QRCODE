...
if [[ "${EUID}" -ne 0 ]]; then
  echo "Run as root: sudo bash scripts/vps/bootstrap-ubuntu24-dual-domain.sh"
  exit 1
fi
...
RESOLVER_DOMAIN="${RESOLVER_DOMAIN:-q.example.com}"
AUTO_DEPLOY_INTERVAL_MINUTES="${AUTO_DEPLOY_INTERVAL_MINUTES:-2}"
ENABLE_TLS="${ENABLE_TLS:-false}"
LETSENCRYPT_EMAIL="${LETSENCRYPT_EMAIL:-}"
ENV_FILE="/etc/qr-forever/qr-forever.env"

if [[ "${APP_DOMAIN}" == "${RESOLVER_DOMAIN}" ]]; then
  echo "APP_DOMAIN and RESOLVER_DOMAIN must be different domains."
  exit 1
fi

log() {
  echo "[qr-forever-dual-bootstrap] $*"
}
...
RESOLVER_DOMAIN=${RESOLVER_DOMAIN}
...
NEXT_PUBLIC_RESOLVER_BASE_URL=https://${RESOLVER_DOMAIN}
NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID=

# Resolver service values
...
systemctl restart qr-forever-resolver.service
REDEPLOY
...
ExecStart=/usr/bin/env pnpm --filter @qr-forever/web start -- -H 127.0.0.1 -p ${WEB_PORT}
Restart=always
RestartSec=5
LimitNOFILE=65535
...
cat > /etc/systemd/system/qr-forever-resolver.service <<SERVICE
[Unit]
Description=qr-forever resolver service
After=network-online.target
Wants=network-online.target
...
WorkingDirectory=${APP_DIR}
Environment=NODE_ENV=production
ExecStart=/usr/bin/env pnpm --filter @qr-forever/resolver-worker exec wrangler dev --config apps/resolver-worker/wrangler.toml --ip 127.0.0.1 --port ${RESOLVER_PORT}
Restart=always
RestartSec=5
LimitNOFILE=65535
...
log "Configuring nginx for two domains"
...
server {
    ...
    location / {
        ...
        proxy_set_header Upgrade \$http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_pass http://127.0.0.1:${RESOLVER_PORT};
    }
}
NGINX
...
if [[ "${ENABLE_TLS}" == "true" ]]; then
  if [[ -z "${LETSENCRYPT_EMAIL}" ]]; then
    log "Skipping TLS: set LETSENCRYPT_EMAIL to enable certbot"
  else
    log "Installing certbot and issuing certificate for both domains"
    apt-get install -y certbot python3-certbot-nginx
    if certbot --nginx -d "${APP_DOMAIN}" -d "${RESOLVER_DOMAIN}" --agree-tos --non-interactive -m "${LETSENCRYPT_EMAIL}" --redirect; then
      log "TLS enabled for ${APP_DOMAIN} and ${RESOLVER_DOMAIN}"
    else
...
systemctl enable qr-forever-resolver.service
FORCE_DEPLOY=1 /usr/local/bin/qr-forever-redeploy.sh
systemctl enable --now qr-forever-deploy.timer

log "Bootstrap complete"
log "Web domain: https://${APP_DOMAIN}"
log "Resolver domain: https://${RESOLVER_DOMAIN}"
log "Edit config: ${ENV_FILE}"
log "Redeploy now: sudo FORCE_DEPLOY=1 /usr/local/bin/qr-forever-redeploy.sh"
log "Logs (web): journalctl -u qr-forever-web -f"
log "Logs (resolver): journalctl -u qr-forever-resolver -f"
